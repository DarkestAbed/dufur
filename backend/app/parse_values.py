...
def get_day_time_of_class(url: str) -> str:
    ...
    logger.logger.debug(f"url: {url} ; dashes: {idxs_dashes} ; slashes: {idxs_slashes}")
...
def get_class_title(text: str) -> str:
    ...
    logger.logger.debug(f"Probando texto: {text_clean}")
    # find the index of string "hrs"
    idx_hrs = text_clean.find("hrs")
    logger.logger.debug(idx_hrs)
    logger.logger.debug(text_clean[idx_hrs + 3:].strip())
    # pdb.set_trace()
    if idx_hrs > 0:
